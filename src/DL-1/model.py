...
class MyModel(nn.Module):
    def __init__(self,hidden_dim, dp_len, maccs_len, ecfp4_len, num_out_dim):
        ...
        self.W_Q2 = nn.Linear(num_out_dim,num_out_dim,bias =False)  # num_out_dim =128
    ...
    def forward(self, DP, MACCS, ECFP4):
        ...
        com_vector = torch.stack((out_dp,out_maccs,out_ecfp4),1)  # [bsz, 3, 128]

        Q2 = self.dropout(self.W_Q2(com_vector))     # [batch_size, 3, 128]
...
def test(flag, model: nn.Module, test_loader, loss_function, device, show):
    ...
    evaluation = {
        ...
        'AUROC': metrics.get_ROC(targets, outputs),
    }
