...
class MyDataset(Dataset):
    def __init__(self,phase, data_path,  dp_len, maccs_len, ecfp4_len, seed):
        ...
        X_test = new_train_data[:531,1:]
        y_test = activity_list[:531]

        over_sampler = SMOTE(random_state=seed)
        X_smote, Y_smote = over_sampler.fit_resample(new_train_data[531:, 1:], activity_list[531:])

        X_train, X_dev, y_train, y_dev = train_test_split(X_smote,Y_smote,test_size=0.06,
                                                          random_state=8888,stratify=Y_smote,shuffle=True)

        if phase == 'training':
            self.activity_list = y_train
            self.dp_value = X_train[:,0:194]
            self.macc_values = X_train[:,194:361]
            self.ecfp4_values = X_train[:,361:2409]
            self.length = X_train.shape[0]


        elif phase == 'validation':
            self.activity_list = y_dev
            self.dp_value = X_dev[:,0:194]
            self.macc_values = X_dev[:,194:361]
            self.ecfp4_values = X_dev[:,361:2409]
            self.length = X_dev.shape[0]

        elif phase == 'test':
            self.activity_list = y_test
            self.dp_value = X_test[:,0:194]
            self.macc_values = X_test[:,194:361]
            self.ecfp4_values = X_test[:,361:2409]
