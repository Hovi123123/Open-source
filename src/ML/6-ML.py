...
def get_data(data_path):
    ...
    new_train_data = np.concatenate((macc1,ecfp1),axis=1)
...
def main(model,train_x,train_y,test_x,test_y,sheet_name,excel_writer):
    ...
    for seed in seed_list:
        ...
        gs = RandomizedSearchCV(clf,parameters, cv=5, scoring=scores, refit='f1', verbose=1, n_jobs=10,n_iter=10)
...
if __name__ == "__main__":
    ...
    excel_writer = pd.ExcelWriter('./6.xlsx')
    sheet_name = 0
    for i in model_list:
